...
- カード上で `Enter` すると、その課題を担当する Claude Code セッションを
  ...
  始められます（[詳細](#相棒セッション)）
- セッションが既にあるカードで `Enter` すると、そのタブへ移動したうえで
  「今どこまで・次は何・何待ち」の3行だけを尋ねます。頭の中で状態を覚えて
  おかなくて済みます（[詳細](#現況の3行)）
...
| `↑` `↓` | 前 / 次のカードへフォーカス |
| `←` `→` | 隣の列へ仮移動 |
| `Enter` | 仮移動を全て確定、または Claude セッション起動（起動済みならそのタブへ移動して現況を確認） |
| `Esc` | 仮移動を全て取消 / 選択解除 |
| `r` | 看板を更新 |
| `o` | 課題をブラウザで開く |
...
`Enter` の動作はセッションの有無で変わります。

- **セッションが無いカード** … セッションを起動し、初期プロンプトの冒頭で
  「今どこまで・次は何・何待ち」の3行を出させてから、作業方針の提案に入らせます。
  根拠は課題の内容と、引き継いだトランスクリプトです。
- **セッションが既にあるカード** … 再起動はせず、そのタブへ移動して同じ3行だけを
  尋ねます。答えるのはそのセッション自身なので、これまでの作業がそのまま根拠に
  なります。

並行している課題の状態を頭の中に置いておかず、戻ってきたときに毎回3行で
思い出せるようにするための機能です。

セッションが応答中（`working`）、または確認待ち（`blocked` / `waiting`）のときは
**何も送らず移動だけ**します。作業中に割り込まないため、また Claude が出している
確認ダイアログに文章で答えてしまわないためです。

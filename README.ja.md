...
- カード上で `Enter` すると、その課題を担当する Claude Code セッションを
  ...
  ボードの隣で開いている Claude Code セッションの記録を引き継ぐので、既に
  話していた内容から始められます（[詳細](#隣のセッションからの引き継ぎ)）
...
`Enter` でセッションを起動するとき、ボードは自分と同じ herdr タブにいる
Claude Code セッション（`HERDR_TAB_ID` と `herdr pane list` で特定）を探し、
その記録（トランスクリプト）のパスを初期プロンプトに追記します。新しい
セッションには「課題キーで grep して該当部分だけ読め」と伝えるので、ボードの
隣で既に話していた内容を打ち直さずに引き継げます。

トランスクリプトはセッションIDで
`${CLAUDE_CONFIG_DIR:-~/.claude}/projects/*/<session-id>.jsonl` から探します。
記録が見つからないペインは対象外で、隣にセッションが無ければ初期プロンプトは
従来どおりです。
